...
msmt_coordinates <- function(
  data_addresses,
  RUIAN_vars = c("RedRUAINKod", "MistoRUAINKod"),
  address_vars = list(
    RedRUAINKod = c("RedAdresa1", "RedAdresa2", "RedAdresa3"),
  ...
  if (!all(all_vars %in% colnames(data_addresses))) {
    stop(paste0(
      "Variables missing in 'data_addresses: ",
      paste0(
        "'",
        all_vars[!all_vars %in% colnames(data_addresses)],
        "'",
        collapse = ", "
      )
    ))
  ...
  data_addresses <- as_tibble(data_addresses) %>%
    mutate(
      ZrizICO = ifelse(nchar(ZrizICO) == 10, substr(ZrizICO, 1, 8), ZrizICO)
    )
  ...
  temp_data_addresses <- lapply(seq_along(RUIAN_vars), function(x) {
    temp_vec <- data_addresses[, RUIAN_vars[x], drop = TRUE]
    temp_ind <- 1:length(temp_vec)
    return_vec <- ifelse(
      is.na(temp_vec),
      paste0(var_marks[x], temp_ind),
      temp_vec
    )
  }) %>%
    `names<-`(var_marks)
  ...
  download.file(destfile = temp_loc, url = ruj_url, quiet = TRUE, mode = "wb")

  options(timeout = old_timeout)

  temp_files <- unzip(temp_loc, exdir = temp_dir)
  ...
  data_ruian <- lapply(seq_along(temp_files), function(x) {
    writeLines(
      iconv(readLines(temp_files[x]), from = "windows-1250", to = "UTF-8"),
      file(temp_files[x], encoding = "UTF-8")
    )
    ...
    progress_bar_1 <- txtProgressBar(min = 0, max = total_1, style = 3)
    progress <- x
  ...
  if (scrape) {
    ...
    progress_bar_2 <- txtProgressBar(min = 0, max = total_2, style = 3)
    ...
    found_codes_scrape <- seq_along(missing_codes_scrape) %>%
      lapply(function(x) {
        ...
        url_lines <- paste0(
          c(
            "https://vdp.cuzk.cz/vdp/ruian/adresnimista/",
        ...
        while (mark) {
          get_lines <- tryCatch(
            {
              readLines(url_lines[y], warn = FALSE)
            },
            error = function(e) {
              return("MISS")
            },
        ...
        if (len_mark > 1) {
          if (sum(grepl("Y:.+X:", get_lines)) > 0) {
            ...
            result <- tibble(RUIAN = temp_RUIAN, X = NA, Y = NA)
          }
        } else {
          result <- tibble(RUIAN = temp_RUIAN, X = NA, Y = NA)
        }
  ...
  found_codes <- found_codes %>%
    sf::st_as_sf(coords = c("X", "Y"), agr = "constant", crs = 5513) %>%
  ...
  if (!is.null(address_vars)) {
    ...
    temp_addresses <- lapply(seq_along(address_vars), function(x) {
    ...
    if (total_3 > 0) {
      ...
      progress_bar_3 <- txtProgressBar(min = 0, max = total_3, style = 3)

      found_RCzechia <- lapply(seq_along(temp_addresses$value), function(x) {
  ...
  temp_coordinates <- lapply(seq_along(RUIAN_vars), FUN = function(x) {
    data_addresses %>%
      left_join(
        found_codes %>%
          `names<-`(c(
  ...
  temp_coordinates[, grepl(
    "source",
    names(temp_coordinates)
  )] <- temp_coordinates[, grepl("source", names(temp_coordinates))] %>%
    mutate_all(function(x) {
      ifelse(is.na(x), "99_failed", x)
    })
  ...
  if (complete) {
    ...
    temp_locations <- lapply(seq_along(RUIAN_vars), FUN = function(x) {
      data_addresses %>%
        left_join(
          data_complete %>%
            `names<-`(ifelse(
